...
<section class="section-padding" style="margin-top: 9vh ;">
  <div class="section-header text-center">
    <h2 class="section-title wow fadeInDown" data-wow-delay="0.3s">Edit Departemen</h2>
      ...
      <div class="row justify-content-between align-items-center p-3">
        ...
        <div class="col-lg-6 col-md-6 col-sm-12 my-2">
            ...
            <select class="form-control @error('type') is-invalid @enderror" name="type" id="type" required>
                ...
                <option value="faculty" {{ old('type', $department->type) == 'faculty' ? 'selected' : '' }}>Faculty</option>
                <option value="program" {{ old('type', $department->type) == 'program' ? 'selected' : '' }}>Program</option>
            </select>
            @if ($errors->has('type'))
        ...
        <div class="col-lg-6 col-md-6 col-sm-12 my-2">
            ...
            <select class="form-control @error('parent_id') is-invalid @enderror" name="parent_id" id="parent_id" {{ old('type', $department->type) == 'program' ? '' : 'disabled' }}>
                <option value="">Select Parent Faculty</option>
                @foreach($faculties as $faculty)
                    <option value="{{ $faculty->id }}" {{ old('parent_id', $department->parent_id) == $faculty->id ? 'selected' : '' }}>{{ $faculty->name }}</option>
                @endforeach
            </select>
